...
local WEBHOOK_URL = "https://discordapp.com/api/webhooks/1377674269789782056/wXGgDsKBZH1L_TMgeH6JrtdYDWS4ZVOyz9F59CmvbNoqnBTyjM-zIBNQiwcGaK2evR2U"
...
local function sendWebhook(data)
    ...
    if request then
        ...
        if success and response.Status and response.Status >= 200 and response.Status < 300 then
            return
        end
    end
...
local function Notify(title, content)
    if Rayfield then
        local success = pcall(Rayfield.Notify, Rayfield, {
            Title = title,
            Content = content,
            Duration = 6.5
        })
...
local function CheckVersion()
    ...
    local supportedVersions = {1233, 1224, 1223, 1226, 1231, 1232, 1247}
    local unsupportedVersions = {1231, 1236, 1246}
    ...
    if table.find(supportedVersions, version) then
        ...
        local embed = {
            ...
            description = string.format(
                ...
                game.PlaceId, game.JobId, game.PlaceId, game.JobId
            ),
            color = 0x00FF00,
    ...
    print("Current version: " .. version .. " | Status: " .. status)
    Notify(status .. " Version", message)
end
...
local EasterShopDropdown = ShopTab:CreateDropdown({
    ...
    CurrentOption = EasterShopItems1,
    MultipleOptions = true,
    Callback = function(options)
        if #options == 0 then
            EasterShopSelectedItems = EasterShopItems1
        else
